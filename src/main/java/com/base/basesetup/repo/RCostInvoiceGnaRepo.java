...
//	Set<Object[]> getStatedetailsFromPartyMaster(Long orgId, String partyCode);
	...
	@Query(value ="SELECT \r\n"
			...
			+ "        JOIN \r\n"
			+ "            accounts c ON a.costinvoiceid = c.sourceid\r\n"
			+ "        WHERE \r\n"
			+ "            a.cancel = 'F'\r\n"
			+ "            AND a.orgid = ?1\r\n"
			+ "            AND (a.suppliercode = ?6 or 'ALL' = ?6)\r\n"
			+ "            AND a.finyear = ?3\r\n"
			+ "            AND (\r\n"
			+ "                ?4 IS NULL \r\n"
			+ "                OR ?4 = '' \r\n"
			+ "                OR ?5 IS NULL \r\n"
			+ "                OR ?5 = ''\r\n"
			+ "                OR c.docdate BETWEEN STR_TO_DATE(?4, '%Y-%m-%d') AND STR_TO_DATE(?5, '%Y-%m-%d')\r\n"
			+ "            )\r\n"
			+ "            AND (a.branchcode = ?2 OR 'ALL' = ?2)\r\n"
			+ "        GROUP BY \r\n"
			+ "            a.branchcode, c.docid, c.docdate, a.docid, a.docdate, \r\n"
			+ "            a.supplietype, a.suppliername, a.supplierbillno, a.suppliergstin, a.gsttype\r\n"
			+ "    ) AS a\r\n"
			+ ") AS b\r\n"
			+ "GROUP BY \r\n"
			+ "    branchcode, docid, docdate, supplietype, suppliername, supplierbillno, suppliergstin, gsttype\r\n"
			+ "ORDER BY \r\n"
			+ "    1, 2, 4, 3\r\n"
			+ " ",nativeQuery =true)
