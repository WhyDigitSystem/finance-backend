...
//			+ "GROUP BY v.orgid")
	...
	@Query(nativeQuery = true,value ="SELECT\r\n"
			+ "    SUM(CASE\r\n"
			+ "            WHEN YEAR(docdate) = YEAR(CURDATE()) AND MONTH(docdate) = MONTH(CURDATE())\r\n"
			+ "            THEN amount\r\n"
			+ "            ELSE 0\r\n"
			+ "        END) AS curmnth,\r\n"
			+ "    SUM(CASE\r\n"
			+ "            WHEN YEAR(docdate) = YEAR(CURDATE() - INTERVAL 1 MONTH) AND MONTH(docdate) = MONTH(CURDATE() - INTERVAL 1 MONTH)\r\n"
			+ "            THEN amount\r\n"
			+ "            ELSE 0\r\n"
			+ "        END) AS premonth,\r\n"
			+ "          	SUM(CASE\r\n"
			+ "            WHEN docdate >= DATE_FORMAT(CURDATE(), '%Y-04-01') \r\n"
			+ "                 AND docdate <= CURDATE()\r\n"
			+ "            THEN amount\r\n"
			+ "            ELSE 0\r\n"
			+ "        END) AS curyear,\r\n"
			+ "    SUM(CASE\r\n"
			+ "            WHEN docdate >= DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 YEAR), '%Y-04-01') \r\n"
			+ "                 AND docdate < DATE_FORMAT(CURDATE(), '%Y-04-01')\r\n"
			+ "            THEN amount\r\n"
			+ "            ELSE 0\r\n"
			+ "        END) AS preyear\r\n"
			+ "FROM vw_revenue where orgid = ?1   \r\n" )
	Set<Object[]> getPercentageDiffFromRevenue(Long orgId);
