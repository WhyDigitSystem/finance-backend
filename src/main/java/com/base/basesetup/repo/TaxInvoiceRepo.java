...
public interface TaxInvoiceRepo extends JpaRepository<TaxInvoiceVO, Long> {
	...
	@Query(nativeQuery =true,value="SELECT \r\n"
			+ "    SUM(a.curmnth) AS current_month, \r\n"
			+ "    SUM(a.premonth) AS previous_month,\r\n"
			+ "    SUM(a.currentyear) AS current_year_total, \r\n"
			+ "    SUM(a.preyear) AS previous_year_total\r\n"
			+ "FROM (\r\n"
			+ "    -- Current Month Revenue\r\n"
			+ "    SELECT v.orgid, SUM(v.amount) AS curmnth, 0 AS premonth, 0 AS currentyear, 0 AS preyear \r\n"
			+ "    FROM vw_revenue v \r\n"
			+ "    WHERE (MONTH(v.docdate) = MONTH(CURDATE()) AND v.finyear = ?2 AND v.orgid = ?1 and ?3='MONTH')\r\n"
			+ "        \r\n"
			+ "    GROUP BY v.orgid\r\n"
			+ "\r\n"
			+ "    UNION \r\n"
			...
			+ "    SELECT v.orgid, 0 AS curmnth, SUM(v.amount) AS premonth, 0 AS currentyear, 0 AS preyear \r\n"
			+ "    FROM vw_revenue v \r\n"
			+ "    WHERE (MONTH(v.docdate) = MONTH(CURDATE())-1 AND v.finyear = ?2 AND v.orgid = ?1 and ?3='MONTH')\r\n"
			+ "    GROUP BY v.orgid\r\n"
			+ "\r\n"
			+ "    UNION \r\n"
			+ "\r\n"
			+ "    -- Current Year Revenue\r\n"
			+ "    SELECT v.orgid, 0 AS curmnth, 0 AS premonth, SUM(v.amount) AS currentyear, 0 AS preyear \r\n"
			+ "    FROM vw_revenue v \r\n"
			+ "    WHERE (YEAR(v.docdate) = YEAR(CURDATE()) )\r\n"
			+ "        AND v.orgid = ?1 and ?3='YEAR'\r\n"
			+ "    GROUP BY v.orgid\r\n"
			+ "\r\n"
			+ "    UNION \r\n"
			+ "\r\n"
			+ "    -- Previous Year Revenue\r\n"
			+ "    SELECT v.orgid, 0 AS curmnth, 0 AS premonth, 0 AS currentyear, SUM(v.amount) AS preyear \r\n"
			+ "    FROM vw_revenue v \r\n"
			+ "    WHERE (YEAR(v.docdate) = YEAR(CURDATE()) - 1)\r\n"
			+ "        AND v.orgid = ?1  and ?3='YEAR'\r\n"
			+ "    GROUP BY v.orgid\r\n"
			+ ") a")
	Set<Object[]> getPercentageDiffFromRevenue(Long orgId,String finYear,String choose);
