...
//	List<TaxInvoiceVO> getCheck(Long orgId, String party);
	...
	@Query(nativeQuery = true, value = "select 1 as sNo,  g.accountgroupname,COALESCE(SUM(a.creditamount), 0) AS totalamount from groupledger g  join accountsdetails a on a.accountname = g.accountgroupname\r\n"
			+ " join accounts a1 on a1.accountsid = a.accountsid  join taxinvoice t on t.docid = a1.refno and t.cancel = 0 where g.coalist = 'INCOME' \r\n"
			+ "and g.type = 'ACCOUNT' and a1.docdate between ?2 and ?3 and a1.orgid=?1 group by g.accountgroupname \r\n"
			+ "union all\r\n"
			+ "select\r\n"
			+ "    2 AS sNo,\r\n"
			+ "    'Income Total' AS accountgroupname,\r\n"
			+ "    COALESCE(SUM(a.creditamount), 0) AS totalamount from groupledger g  join accountsdetails a on a.accountname = g.accountgroupname\r\n"
			+ " join accounts a1 on a1.accountsid = a.accountsid  join taxinvoice t on t.docid = a1.refno and t.cancel = 0 where g.coalist = 'INCOME' \r\n"
			+ "and g.type = 'ACCOUNT'  and a1.docdate between ?2 and ?3 and a1.orgid=?1\r\n"
			+ "union all\r\n"
			+ "select 3 as sNo,  g.accountgroupname,COALESCE(SUM(a.debitamount), 0) AS totalamount from groupledger g  join accountsdetails a on a.accountname = g.accountgroupname\r\n"
			+ " join accounts a1 on a1.accountsid = a.accountsid left join costinvoice t on t.docid = a1.refno left join urcostinvoicegna u on u.docid = a1.refno \r\n"
			+ " left join rcostinvoicegna r on r.docid = a1.refno\r\n"
			+ " and t.cancel = 0 where g.coalist = 'EXPENSE' \r\n"
			+ "and g.type = 'ACCOUNT'  and a1.docdate between ?2 and ?3 and a1.orgid=?1 group by g.accountgroupname \r\n"
			+ "union all\r\n"
			+ "select 4 as sNo,  'Expence Total' AS accountgroupname,COALESCE(SUM(a.debitamount), 0) AS totalamount from groupledger g  join accountsdetails a on a.accountname = g.accountgroupname\r\n"
			+ " join accounts a1 on a1.accountsid = a.accountsid left join costinvoice t on t.docid = a1.refno left join urcostinvoicegna u on u.docid = a1.refno \r\n"
			+ " left join rcostinvoicegna r on r.docid = a1.refno\r\n"
			+ " and t.cancel = 0 where g.coalist = 'EXPENSE' \r\n"
			+ "and g.type = 'ACCOUNT'  and a1.docdate between ?2 and ?3 and a1.orgid=?1\r\n"
			+ "union all\r\n"
			+ "select 5 as sNo,\r\n"
			+ "       'Profit/Loss' AS accountgroupname,\r\n"
			+ "       ( (select COALESCE(SUM(a.creditamount),0)\r\n"
			+ "          from groupledger g  \r\n"
			...
			+ "          where g.coalist = 'INCOME' and g.type = 'ACCOUNT' and a1.docdate between ?2 and ?3 and a1.orgid=?1)\r\n"
			+ "        -\r\n"
			+ "        (select COALESCE(SUM(a.debitamount),0)\r\n"
			+ "          from groupledger g  \r\n"
			+ "          join accountsdetails a on a.accountname = g.accountgroupname\r\n"
			+ "          join accounts a1 on a1.accountsid = a.accountsid  \r\n"
			...
			+ "          left join rcostinvoicegna r on r.docid = a1.refno  \r\n"
			+ "          and t.cancel = 0  \r\n"
			+ "          where g.coalist = 'EXPENSE' and g.type = 'ACCOUNT' and a1.docdate between ?2 and ?3 and a1.orgid=?1)\r\n"
			+ "       ) AS totalamount\r\n"
			+ "\r\n"
			+ "order by sNo, accountgroupname")
