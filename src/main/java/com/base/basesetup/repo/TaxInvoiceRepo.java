...
//			+ "GROUP BY v.orgid")
	...
	@Query(nativeQuery = true,value ="SELECT \r\n"
			...
			+ "    \r\n"
			+ ") a")
	Set<Object[]> getPercentageDiffFromRevenue(Long orgId,Long finYear,String month,String year);

	@Query(nativeQuery =true,value = "    select sum(a.curyear) as currentyear,sum(a.preyear) as previousyear from (SELECT \r\n"
			+ "    SUM(v.amount) AS curyear, \r\n"
			+ "    0 AS preyear, \r\n"
			...
			+ "FROM vw_revenue v\r\n"
			+ "WHERE \r\n"
			+ "    (MONTH(v.docdate) >= 4 AND cast(v.finyear as signed)=2024) \r\n"
			+ "    OR (MONTH(v.docdate) < 4 AND cast(v.finyear as signed)=2025) and orgid=202502 \r\n"
			...
			+ "FROM vw_revenue v\r\n"
			+ "WHERE \r\n"
			+ "    (MONTH(v.docdate) >= 4 AND (cast(v.finyear as signed)=?2)-1) \r\n"
			+ "    OR (MONTH(v.docdate) < 4 AND cast(v.finyear as signed)=?2)  and orgid=?1\r\n"
			+ "GROUP BY MONTH(v.docdate), v.finyear,v.orgid\r\n"
			+ ") a")
	Set<Object[]> getPercentageDiffFromYear(Long orgId, Long finYear);
	
    @Query(nativeQuery =true,value = "SELECT SUM(a.curyear) AS currentyear,SUM(a.preyear) AS previousyear,SUM(a.curmonth) AS currentmonth,SUM(a.premonth) AS previousmonth FROM (\r\n"
    		+ "SELECT SUM(r.paymentamt) AS curyear,0 AS preyear,0 AS curmonth,0 AS premonth,r.orgid FROM payment r\r\n"
    		+ "WHERE r.finyear =?2 \r\n"
    		+ "AND r.orgid = ?1 AND ?3 = 'YEAR' \r\n"
    		+ "GROUP BY r.orgid \r\n"
    		+ "UNION \r\n"
    		+ "SELECT 0 AS curyear,SUM(r.paymentamt) AS preyear,0 AS curmonth,0 AS premonth,r.orgid FROM payment r \r\n"
    		+ "WHERE r.finyear =(?2 -1)\r\n"
    		+ "AND r.orgid = ?1 AND ?3 = 'YEAR' \r\n"
    		+ "GROUP BY r.orgid \r\n"
    		+ "UNION \r\n"
    		+ "SELECT 0 AS curyear,0 AS preyear,SUM(r.paymentamt) AS curmonth,0 AS premonth,r.orgid FROM payment r\r\n"
    		+ "JOIN financialyear f ON f.finyear = r.finyear \r\n"
    		+ "WHERE r.docdate BETWEEN f.startdate AND f.enddate AND MONTH(r.docdate) = MONTH(CURDATE()) AND CAST(r.finyear AS SIGNED) = ?2 AND r.orgid = ?1 AND ?3 = 'MONTH' \r\n"
    		+ "GROUP BY r.orgid \r\n"
    		+ "union\r\n"
    		+ "SELECT 0 AS curyear,0 AS preyear,0 AS curmonth,SUM(r.paymentamt) AS premonth,r.orgid FROM payment r \r\n"
