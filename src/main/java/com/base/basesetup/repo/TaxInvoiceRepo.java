...
//			+ "GROUP BY v.orgid")
	...
	@Query(nativeQuery = true,value ="SELECT \r\n"
			...
			+ "        AND MONTH(v.docdate) = \r\n"
			+ "        CASE \r\n"
			+ "            WHEN MONTH(CURDATE()) = 4 THEN 12  -- Instead of NULL, return 12 (previous year December)\r\n"
			+ "            ELSE MONTH(CURDATE()) - 1\r\n"
			+ "        END\r\n"
			+ "        AND v.orgid = ?1\r\n"
			+ "        AND ?3='MONTH'\r\n"
			...
			+ "    FROM vw_revenue v\r\n"
			+ "    WHERE \r\n"
			+ "        ((MONTH(v.docdate) >= 4 AND CAST(v.finyear AS SIGNED) = ?2) \r\n"
			+ "        OR (MONTH(v.docdate) < 4 AND CAST(v.finyear AS SIGNED) = (?2 + 1)))\r\n"
			+ "        AND v.orgid = ?1\r\n"
			+"         AND ?4='YEAR' \r\n "
			+ "    GROUP BY v.orgid\r\n"
			+ "\r\n"
			...
			+ "        OR (MONTH(v.docdate) < 4 AND CAST(v.finyear AS SIGNED) = ?2))\r\n"
			+ "        AND v.orgid = ?1\r\n"
			+"         AND ?4='YEAR' \r\n "
			+ "    GROUP BY v.orgid\r\n"
			+ ") a\r\n"
