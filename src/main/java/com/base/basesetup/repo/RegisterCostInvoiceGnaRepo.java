...
public interface RegisterCostInvoiceGnaRepo extends JpaRepository<RegisterCostInvoiceGnaVO, Long> {
	...
	@Query(nativeQuery = true, value = "select a.sectionname ,b.tcspercentage  from tdsmaster a , tdsmaster2 b where a.tdsmasterid=b.tdsmasterid and a.orgid=?1 and a.section=?2 and a.active = 1  order by sectionname")
	...
	@Query(nativeQuery = true, value = "SELECT\r\n"
			+ "    ROW_NUMBER() OVER (ORDER BY evid, evdate) AS sno,\r\n"
			+ "    branchcode,\r\n"
			+ "    evid,\r\n"
			+ "    evdate,\r\n"
			+ "    suppliername,\r\n"
			...
			+ "    SUM(tds) AS tds,\r\n"
			+ "    SUM(partyPayable) AS partyPayable,\r\n"
			+ "    SUM(igst) AS Output_IGST,\r\n"
			+ "    SUM(cgst) AS Output_CGST,\r\n"
			+ "    SUM(sgst) AS Output_SGST,\r\n"
			...
			+ "FROM\r\n"
			+ "(\r\n"
			+ "    -- Cost Invoice Data (Positive Values) - FIXED: Pre-aggregate to avoid duplicates\r\n"
			+ "    SELECT\r\n"
			+ "        a.branchcode,\r\n"
			+ "        a.vid AS evid,\r\n"
			+ "        a.vdate AS evdate,\r\n"
			...
			+ "        a.suppliergstin,\r\n"
			+ "        a.gsttype,\r\n"
			+ "        a.totchargeslcamt AS billamount,\r\n"
			+ "        (a.totchargeslcamt + a.gstinputlcamt) AS totalamount,\r\n"
			+ "        d.totaltds AS tds,\r\n"
			+ "        a.netbilllcamt AS partyPayable,\r\n"
			+ "        CASE WHEN a.gsttype='INTER' THEN a.gstinputlcamt ELSE 0 END AS igst,\r\n"
			+ "        CASE WHEN a.gsttype='INTRA' THEN a.gstinputlcamt/2 ELSE 0 END AS cgst,\r\n"
			+ "        CASE WHEN a.gsttype='INTRA' THEN a.gstinputlcamt/2 ELSE 0 END AS sgst,\r\n"
			+ "        MAX(b.gstpercent) AS gstpercent,\r\n"
			+ "        a.docid,\r\n"
			+ "        a.docdate,\r\n"
			+ "        a.screencode\r\n"
			+ "    FROM costinvoice a\r\n"
			...
			+ "    UNION ALL\r\n"
			+ "\r\n"
			+ "    -- Cost Debit Note Data (Negative Values) - FIXED: Pre-aggregate to avoid duplicates\r\n"
			+ "    SELECT\r\n"
			+ "        a.branchcode,\r\n"
			+ "        a.vid AS evid,\r\n"
			+ "        a.vdate AS evdate,\r\n"
			...
			+ "        a.suppliergstin,\r\n"
			+ "        a.gsttype,\r\n"
			+ "        a.totchargeslcamt * -1 AS billamount,\r\n"
			+ "        (a.totchargeslcamt + a.gstinputlcamt) * -1 AS totalamount,\r\n"
			+ "        d.totaltds * -1 AS tds,\r\n"
			+ "        a.netbilllcamt * -1 AS partyPayable,\r\n"
			+ "        CASE WHEN a.gsttype='INTER' THEN a.gstinputlcamt * -1 ELSE 0 END AS igst,\r\n"
			+ "        CASE WHEN a.gsttype='INTRA' THEN (a.gstinputlcamt/2) * -1 ELSE 0 END AS cgst,\r\n"
			+ "        CASE WHEN a.gsttype='INTRA' THEN (a.gstinputlcamt/2) * -1 ELSE 0 END AS sgst,\r\n"
			+ "        MAX(b.gstpercent) AS gstpercent,\r\n"
			+ "        a.docid,\r\n"
			+ "        a.docdate,\r\n"
			+ "        a.screencode\r\n"
			+ "    FROM costdebitnote a\r\n"
			+ "    JOIN chargercostdebitnote b ON a.costdebitnoteid=b.costdebitnoteid\r\n"
			+ "    JOIN accounts c ON a.docid=c.refno AND a.vid=c.vid\r\n"
			+ "    JOIN costdebitnotetaxprtcul d ON a.costdebitnoteid=d.costdebitnoteid\r\n"
			+ "    WHERE a.cancel='F'\r\n"
			+ "      AND a.orgid=?1\r\n"
			+ "      AND (?2='ALL' OR a.branchcode=?2)\r\n"
			+ "      AND ((?3 IS NULL AND ?4 IS NULL) OR c.vdate BETWEEN DATE(?3) AND DATE(?4))\r\n"
			...
			+ "UNION ALL\r\n"
			+ "\r\n"
			+ "-- TOTAL SUMMARY ROW - FIXED: Pre-aggregate to avoid duplicates\r\n"
			+ "SELECT\r\n"
			+ "    NULL AS sno,\r\n"
			+ "    'TOTAL' AS branchcode,\r\n"
			+ "    NULL AS evid,\r\n"
			...
			+ "FROM\r\n"
			+ "(\r\n"
			+ "    -- Cost Invoice Data (Positive) - FIXED: Pre-aggregate\r\n"
			+ "    SELECT\r\n"
			+ "        a.branchcode,\r\n"
			+ "        a.vid AS evid,\r\n"
			+ "        a.vdate AS evdate,\r\n"
			...
			+ "    UNION ALL\r\n"
			+ "\r\n"
			+ "    -- Cost Debit Note Data (Negative) - FIXED: Pre-aggregate with GST negation\r\n"
			+ "    SELECT\r\n"
			+ "        a.branchcode,\r\n"
			+ "        a.vid AS evid,\r\n"
			+ "        a.vdate AS evdate,\r\n"
			...
			+ ") total_data\r\n"
			+ "ORDER BY \r\n"
			+ "    CASE WHEN branchcode = 'TOTAL' THEN 1 ELSE 0 END,\r\n"
			+ "    evid, \r\n"
			+ "    evdate")
