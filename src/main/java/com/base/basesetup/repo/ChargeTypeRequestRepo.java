...
public interface ChargeTypeRequestRepo extends JpaRepository<ChargeTypeRequestVO, Long> {
	...
	@Query(nativeQuery = true, value = "select upper(accountgroupname) from groupledger where active=1 and category in('RECEIVABLE A/C') and type='ACCOUNT' and orgid=?1")
	Set<Object[]> findSalesAccountFromGroup(Long orgId);

	@Query(nativeQuery = true, value = "select upper(accountgroupname) from groupledger where active=1 and category in('PAYABLE A/C') and type='ACCOUNT' and orgid=?1")
