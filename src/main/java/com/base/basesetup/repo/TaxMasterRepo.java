...
public interface TaxMasterRepo extends JpaRepository<TaxMasterVO,Long>{
	...
	@Query(nativeQuery = true,value = "select accountgroupname from groupledger where orgid=?1 and type in ('ACCOUNT') and category in ('RECEIVABLE A/C') and gsttaxflag in ('INPUT TAX') and active=1")
	Set<Object[]> findRevenueLegderForTaxMaster(Long orgId);

	@Query(nativeQuery = true,value = "select accountgroupname from groupledger where orgid=?1 and type in ('ACCOUNT') and category in ('PAYABLE A/C') and gsttaxflag in ('INPUT TAX') and active=1")
