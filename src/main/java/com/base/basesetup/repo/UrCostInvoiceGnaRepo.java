...
public interface UrCostInvoiceGnaRepo extends JpaRepository<UrCostInvoiceGnaVO, Long> {
	...
	@Query(nativeQuery = true, value = "select  accountgroupname from groupledger where orgid=?1 and gsttaxflag!='NA' and \r\n"
	...
	@Query(nativeQuery = true, value = "select accountgroupname from groupledger where accountgroupname=?1")
