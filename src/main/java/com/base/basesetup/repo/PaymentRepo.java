...
public interface PaymentRepo extends JpaRepository<PaymentVO, Long> {
	...
	@Query(nativeQuery =true,value = "WITH b AS (\r\n"
			...
			+ "JOIN costinvoice d \r\n"
			+ "    ON a.refno = d.docid \r\n"
			+ "JOIN chargercostinvoice t \r\n"
			+ "    ON d.costinvoiceid = t.costinvoiceid \r\n"
			+ "JOIN accounts c \r\n"
			+ "    ON a.docid = c.docid\r\n"
			+ "JOIN costdebitnote h \r\n"
			+ "    ON d.docid = h.orginbill\r\n"
			+ "LEFT JOIN n \r\n"
			+ "    ON n.orgid = a.orgid \r\n"
			+ "    AND n.refno = c.vid\r\n"
			+ "WHERE \r\n"
			...
			+ "    a.tdsamt,\r\n"
			+ "    t.gstpercent,\r\n"
			+ "    n.settamt having SUM(d.totchargeslcamt - h.totchargeslcamt) - COALESCE(n.settamt, 0)>0\r\n"
			+ "ORDER BY \r\n"
			+ "    a.refdate,\r\n"
			+ "    c.vid")
