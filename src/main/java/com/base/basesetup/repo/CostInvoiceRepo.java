...
//			+ "")
	...
	@Query(nativeQuery =true,value="SELECT SUM(a.curyear) AS currentyear,SUM(a.preyear) AS previousyear,SUM(a.curmonth) AS currentmonth,SUM(a.premonth) AS previousmonth FROM (\r\n"
			+ "SELECT SUM(r.receiptamt) AS curyear,0 AS preyear,0 AS curmonth,0 AS premonth,r.orgid FROM receipt r\r\n"
			+ "WHERE r.finyear =?2 \r\n"
			+ "AND r.orgid = ?1 AND ?3 = 'YEAR' \r\n"
			+ "GROUP BY r.orgid \r\n"
			+ "UNION \r\n"
			+ "SELECT 0 AS curyear,SUM(r.receiptamt) AS preyear,0 AS curmonth,0 AS premonth,r.orgid FROM receipt r \r\n"
			+ "WHERE r.finyear =(?2 -1)\r\n"
			+ "AND r.orgid = ?1 AND ?3 = 'YEAR' \r\n"
			+ "GROUP BY r.orgid \r\n"
			+ "UNION \r\n"
			+ "SELECT 0 AS curyear,0 AS preyear,SUM(r.receiptamt) AS curmonth,0 AS premonth,r.orgid FROM receipt r\r\n"
			+ "JOIN financialyear f ON f.finyear = r.finyear \r\n"
