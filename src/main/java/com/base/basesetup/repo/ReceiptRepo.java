...
public interface ReceiptRepo extends JpaRepository<ReceiptVO, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT \r\n"
	        ...
	        + "    LEFT JOIN branch br ON p.branch = br.branchid\r\n"
	        + "        COALESCE(p.cancel, 0) = 1  \r\n"
	        + "      AND (LOWER(COALESCE(p.partycode, '')) = LOWER(?2) OR 'ALL' = ?2)\r\n"  
	        + "        AND (p.docdate BETWEEN date(?5) AND date(?6) OR ( 'NULL'=?5 AND 'NULL'=?6))\r\n"
	        + "      AND (LOWER(COALESCE(br.branchcode, '')) = LOWER(?3) OR 'ALL' = ?3)\r\n"
	        + "      AND p.orgid = ?1\r\n" 
	        + "      AND (p.finyear = ?4 OR 'ALL' = ?4)\r\n" 
	        + "    ) AS a \r\n"
	...
	Set<Object[]> getPaymentRegisterReport(Long orgId, String partyName, String branchCode, String finYear,
	        String fromDate, String toDate);
