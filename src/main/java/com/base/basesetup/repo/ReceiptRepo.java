...
public interface ReceiptRepo extends JpaRepository<ReceiptVO, Long> {
	...
	@Query(nativeQuery =true,value ="SELECT r.orgid, r.branchcode, r.finyear, \r\n"
			...
			+ "           CASE WHEN tds_rank = 1 THEN r.tdsamt ELSE 0 END AS tdsamount, \r\n"
			...
			+ "        FROM receipt r \r\n"
			+ "        WHERE COALESCE(r.cancel, 0) = 1  \r\n"
			+ "          AND (LOWER(COALESCE(r.customername, '')) = LOWER(?2) OR ?2 = 'ALL') \r\n"
			+ "          AND (LOWER(COALESCE(r.branchcode, '')) = LOWER(?3) OR ?3 = 'ALL')  \r\n"
			+ "          AND (\r\n"
			+ "                (?5 IS NOT NULL AND ?6 IS NOT NULL AND r.docdate BETWEEN DATE(?5) AND DATE(?6)) \r\n"
