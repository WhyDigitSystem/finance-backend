...
public interface PartyMasterRepo extends JpaRepository<PartyMasterVO, Long> {
	...
	@Query(nativeQuery = true, value = "WITH \r\n"
			...
			+ "    t1.CANCEL = 0\r\n"
			+ "    AND (COALESCE(?4, '') = '' \r\n"
			+ "         OR t1.docdate = STR_TO_DATE(?4, '%Y-%m-%d'))\r\n"
			...
			+ "    (SELECT subledgername FROM accountsdetails WHERE accountsid = t1.accountsid AND subledgername NOT IN ('None')) AS partyname,\r\n"
			+ "    0 AS opbal, \r\n"
			+ "    t1.currency, \r\n"
			+ "    SUM(t2.bdebitamount) AS dbamount, \r\n"
			+ "    SUM(t2.bcreditamount) AS cramount, \r\n"
			...
			+ "    AND (\r\n"
			+ "        COALESCE(?4, '') = '' \r\n"
			+ "        OR t1.docdate BETWEEN STR_TO_DATE(?4, '%Y-%m-%d') AND STR_TO_DATE(?5, '%Y-%m-%d')\r\n"
			+ "        OR COALESCE(?5, '') = ''\r\n"
			+ "    )\r\n"
			+ "    AND (?3 = 'ALL' OR b.branchcode =?3)\r\n"
			+ "    AND t1.orgid =?1\r\n"
