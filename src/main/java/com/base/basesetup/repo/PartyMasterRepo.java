...
public interface PartyMasterRepo extends JpaRepository<PartyMasterVO, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT\r\n"
			...
			+ "        ELSE COALESCE(SUM(t2.bcreditamount), 0) - COALESCE(SUM(t2.bdebitamount), 0)\r\n"
			+ "    END AS opbal,\r\n"
			+ "        CASE \r\n"
			+ "	   WHEN ?5 IS NULL THEN 0\r\n"
			+ "			    ELSE COALESCE(SUM(t2.bcreditamount), 0) - COALESCE(SUM(t2.bdebitamount), 0)\r\n"
			...
			+ "        (SUM(t2.bcreditamount) - SUM(t2.bdebitamount)) AS clbal\r\n"
			+ "    FROM\r\n"
			+ "        accountsdetails t2\r\n"
			+ "        JOIN accounts t1 ON t1.accountsid = t2.accountsid\r\n"
			+ "        JOIN partymaster p ON t2.subledgercode = p.partycode\r\n"
			...
			+ "        t1.CANCEL = 0\r\n"
			+ "        AND (\r\n"
			+ "            (t1.vdate BETWEEN DATE(?5) AND DATE(?6))\r\n"
			+ "            OR (?5 IS NULL AND ?6 IS NULL)\r\n"
			+ "        )\r\n"
			+ "        AND (p.partyname = ?2 OR ?2 = 'ALL')\r\n"
			+ "        AND (p.partytype = ?3 OR ?3 = 'ALL' AND ?3 = 'ALL' OR ?3 = 'ALL')\r\n"
			+ "        AND t2.acategory IN ('PAYABLE A/C', 'RECEIVABLE A/C')\r\n"
			...
			+ "        NULL AS currency,\r\n"
			+ "        NULL AS opbal,\r\n"
			+ "        (SUM(t2.bcreditamount) - SUM(t2.bdebitamount)) AS dbamount,\r\n"
			+ "        0 AS cramount, \r\n"
			+ "        0 AS billdbamount,\r\n"
			+ "        0 AS billcramount,\r\n"
			+ "        (SUM(t2.bcreditamount) - SUM(t2.bdebitamount)) AS clbal\r\n"
	...
	@Query(nativeQuery = true, value = "select partycode,partyname,partyshortname from partymaster where orgid=?1 and bussinesscate='TRANSPORTERS'")
